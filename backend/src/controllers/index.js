...
import { env, assertCloudinaryEnv } from '../config/env.js';
...
export const me = async (req, res) =>
  ok(res, {
    user: await currentUser(req.user)
  });

export const profilePhotoSignature = async (req, res) => {
  assertCloudinaryEnv();
  ...
  const signature =
    createCloudinarySignature(parameters);
...
export const profilePhotoUpdate = async (req, res) => {
  ...
  if (!secureUrl.startsWith(expectedUrlPrefix)) {
    throw new HttpError(
      400,
...
export const sellerApply = async (req, res) =>
  created(
    res,
    await applySeller(
      req.user.uid,
...
export const orderDone = async (req, res) =>
  ok(
    res,
    await doneOrder(
      req.query.id,
...
export const sellerCall = async (req, res) =>
  ok(
    res,
    await callSeller(req.query.id)
  );

export const dashboard = async (req, res) =>
  ok(res, await A.dashboard());
