...
async function resolveUser(identifier, hints = {}) {
  ...
  if (resolvedUid) {
    ...
    if (authSeller) {
      seller = authSeller.seller;
      sellerRef = authSeller.sellerRef;
    }
    if (authRegistration) {
      registration = authRegistration.registration;
      registrationRef = authRegistration.registrationRef;
    }
  }
  if (!sellerRef) {
  ...
  if (resolvedUid && !sellerRef) {
    ...
    if (resolvedSeller) {
      seller = resolvedSeller.seller;
      sellerRef = resolvedSeller.sellerRef;
    }
  }
  if (resolvedUid && !registrationRef) {
    ...
    if (resolvedRegistration) {
      registration = resolvedRegistration.registration;
      registrationRef = resolvedRegistration.registrationRef;
    }
  }
...
async function resolveSeller(uid) {
  const normalizedUid = normalize(uid);
  if (!normalizedUid) {
    return { seller: null, sellerRef: null, registration: null, registrationRef: null };
  }
  ...
  if (!sellerRef) {
    ...
    if (legacySeller) {
      seller = legacySeller.data() || {};
      sellerRef = legacySeller.ref;
    }
  }
  if (!registrationRef) {
    const legacyRegistration = await findByIdOrUid('registrations', [normalizedUid]);
    if (legacyRegistration) {
      registration = legacyRegistration.data() || {};
      registrationRef = legacyRegistration.ref;
    }
  }
  if (!isApprovedSeller(seller)) {
    ...
    if (approvedSeller) {
      seller = approvedSeller.data() || {};
      sellerRef = approvedSeller.ref;
    }
  }
  if (!registration || registration.status !== 'approved') {
    ...
    if (approvedRegistration) {
      registration = approvedRegistration.data() || {};
      registrationRef = approvedRegistration.ref;
    }
  }
...
export async function listCollection(name) {
  ...
  const normalizedUsers = await Promise.all(items.map(async user => {
    ...
    return {
      ...
      uid,
      role,
      sellerActive: sellerApproved,
      sellerStatus: seller?.status || null,
      sellerBanned: Boolean(seller?.banned)
    };
  }));
...
export async function setStatus(type, id, status) {
  if (!id) {
    throw new HttpError(400, 'ID data wajib diisi.');
  }
  if (type === 'orders') {
  ...
  if (type === 'rooms') {
    throw new HttpError(400, 'Room tidak menggunakan status approve/reject. Gunakan lifecycle transaksi.');
  }
  const collection = COLLECTIONS[type];
  if (!collection) {
    throw new HttpError(400, 'Jenis tidak valid.');
  }
  const allowed = {
  ...
  if (type === 'sellers') {
    ...
    if (registration.status !== 'pending') {
      throw new HttpError(409, 'Pengajuan seller ini sudah diproses.');
    }
    if (status === 'approved') {
      ...
      batch.set(sellerRef, {
        ...
        description: registration.description || registration.reason || '',
  ...
  if (type === 'orders' && status === 'cancelled') {
    return db.runTransaction(async transaction => {
      ...
      if (current.status !== 'in_transaction') {
      ...
      const [product, room, payment] = await Promise.all([
  ...
  if (type === 'payments') {
    ...
    if (payment.status !== 'pending') {
      throw new HttpError(409, 'Pembayaran ini sudah diproses.');
    }
...
export async function setBan(uid, banned) {
  ...
  if (!identifier) {
    throw new HttpError(400, 'UID user wajib diisi.');
  }
  ...
  if (isAdminUid(normalizedUid)) {
    throw new HttpError(403, 'Akun admin utama tidak dapat diban.');
  }
...
export async function updateUser(uid, { banned, role, name, phone, userId, id, email } = {}) {
  ...
  if (!resolved.uid) {
    try {
      ...
      if (canonicalUid) {
        resolved = await resolveUser(canonicalUid, { uid: canonicalUid, email: foundData?.email || email });
      }
    } catch {}
  }
  if (!resolved.uid) {
  ...
  if (hasName || hasPhone) {
    ...
    if (!cleanName) {
      throw new HttpError(400, 'Nama seller wajib diisi.');
    }
    const batch = db.batch();
  ...
  if (role !== undefined) {
    if (!['buyer', 'seller'].includes(role)) {
      throw new HttpError(400, 'Role tidak valid.');
    }
    if (role === 'seller') {
...
export async function saveSettings(data) {
  ...
  if (qrisUrl) {
    try {
      ...
      throw new HttpError(400, 'URL QRIS tidak valid.');
    }
  }
