...
async function adminUids(){ const out=[]; for(const email of env.adminEmails){ try{out.push((await auth.getUserByEmail(email)).uid)}catch{} } return [...new Set(out)]; }
export async function createOrder(buyerUid, productId){
 const admins=await adminUids(); if(!admins.length) throw new HttpError(503,'Admin belum dikonfigurasi.');
 return db.runTransaction(async tx=>{
  const productRef=db.collection('products').doc(productId), snap=await tx.get(productRef); if(!snap.exists) throw new HttpError(404,'Produk tidak ditemukan.'); const p=snap.data(), stock=Number(p.stock);
...
export async function doneOrder(orderId,buyerUid){ return db.runTransaction(async tx=>{
 const orderRef=db.collection('orders').doc(orderId), os=await tx.get(orderRef); if(!os.exists) throw new HttpError(404,'Order tidak ditemukan.'); const o=os.data();
 if(o.buyerUid!==buyerUid) throw new HttpError(403,'Hanya buyer yang dapat menekan DONE.'); if(['completed','cancelled'].includes(o.status)) throw new HttpError(409,'Order sudah ditutup.'); if(o.paymentStatus!=='verified') throw new HttpError(409,'Pembayaran belum diverifikasi Admin.');
 const productRef=db.collection('products').doc(o.productDocId||o.productId), ps=await tx.get(productRef), roomRef=db.collection('rooms').doc(orderId); tx.update(orderRef,{status:'completed',stockReserved:false,completedAt:FieldValue.serverTimestamp(),updatedAt:FieldValue.serverTimestamp()});
 ...
 if(ps.exists){const p=ps.data(), next=Math.max(0,Number(p.stock)-1); tx.update(productRef,next===0?{stock:0,status:'sold',visibility:'private',updatedAt:FieldValue.serverTimestamp()}:{stock:next,status:'available',visibility:'public',updatedAt:FieldValue.serverTimestamp()});}
 return {ok:true};
 });}
